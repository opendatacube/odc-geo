...
from datacube.model import DatasetType, GridSpec, Measurement, MetadataType
...
def test_product_nodata_nan():
    ...
    product = mk_sample_product(
        "test",
        measurements=[
            dict(name="_nan", dtype="float32", nodata="NaN"),
            dict(name="_inf", dtype="float32", nodata="Infinity"),
            dict(name="_neg_inf", dtype="float32", nodata="-Infinity"),
        ],
    )
...
def test_product_scale_factor():
    product = mk_sample_product(
        "test", measurements=[dict(name="red", scale_factor=33, add_offset=-5)]
    )
...
def test_product_load_hints():
    product = mk_sample_product(
        "test_product", load=dict(crs="epsg:3857", resolution={"x": 10, "y": -10})
    )
    ...
    product = mk_sample_product(
        "test_product",
        load=dict(
            crs="epsg:3857", align={"x": 5, "y": 6}, resolution={"x": 10, "y": -10}
        ),
    )
    ...
    product = mk_sample_product(
        "test", storage=dict(crs="EPSG:3857", resolution={"x": 10, "y": -10})
    )
    assert product.grid_spec is None
    assert product.default_resolution == (-10, 10)
    assert product.default_crs == geometry.CRS("EPSG:3857")
    ...
    product = mk_sample_product("test", storage=dict(crs="EPSG:3857"))
    assert product.grid_spec is None
    assert product.load_hints() == {}
    ...
    product = mk_sample_product(
        "test_product",
        load=dict(crs="epsg:4326", resolution={"longtude": 1.2, "latitude": -1.1}),
    )
    assert product.load_hints() == {}
