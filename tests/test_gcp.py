...
def test_gcp_geobox_basics(au_gcp_geobox: GCPGeoBox) -> None:
    ...
    assert p1 == pytest.approx((-44.49609375, 109.40631987913493), abs=1e-4)
    assert p2 == pytest.approx((-9.456647862641567, 157.05555456254376), abs=1e-4)
    ...
    assert p3 == pytest.approx(p1, abs=1e-4)
    assert p4 == pytest.approx(p2, abs=1e-4)
